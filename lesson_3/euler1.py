...
def multiples():
    return sum(list(filter((lambda x: x % 3 == 0 or x % 5 == 0), range(1, 1000))))
...
def fibonacci():
    fib = list(gen_fibi(4000000))
    return sum(list(filter((lambda x: x % 2 == 0), fib)))
...
def palindrome():
   # ls=list(map(lambda x,y:x*y,range(100,1000),range(100,1000)))
    ls=[]
    for j in range(100,1000):
        for i in range(100,1000):
            ls.append(i*j)
    
    return max(list(filter((lambda x: str(x) == str(x)[::-1]), ls)))
