...
def diff():
    # +++ ваш код +++
    kv1 = reduce((lambda x, y: x + y), range(1,101)) ** 2
    kv2 = reduce((lambda x, y: x + y**2), range(1,101))    
    return kv1 - kv2
...
chislo1000 = ""

def five_str(i):
    stroka = chislo1000[i:i+5]
    proizv = reduce((lambda x, y: x*y), map(lambda x: int(x),str(stroka))) 
    return proizv

def max_five():
    # +++ ваш код +++
    ls =[]
    for i in range(0,len(chislo1000)-4):
        ls.append(five_str(i))
    return max(ls)
