...
def extract_names(filename):
    ...
    reg = re.compile(r'''"> 
                    (.*?)</td>\s*?''',re.VERBOSE | re.DOTALL)

    """
    так почемуто не срабатывает - всяко пробывал
    reg = re.compile(r'''<tr> <td width="66"> 
        (.*?)
        </td>\s*? <td width="151"> 
        (.*?)
        </td>\s*? <td width="85"> 
...
def print_names(babynames):
    # +++ваш код+++
    year = input('Введите год (q - выход): ')
    while year != 'q':
        ...
        if not year in years:
            ...
            for key in sorted(babynames.keys()):
                print("{0}: {1}".format(key,babynames[key][index]))
        year = input('Введите год (q - выход): ')
    print('Выход')
    sys.exit(1)
    return
