...
def match_ends(words):      # 2016.03.17_12:17:12 checked. prusanov
    # +++ ваш код +++
    sum = 0
    for st in words:
        if len(st) > 1 and st[0] == st[-1]:             # эти два условия можно объединить в один if
                sum +=1        
    return sum
...
def front_x(words):     # 2016.03.17_12:17:22 checked. prusanov
    # +++ ваш код +++
    a = []
    b = []
    for st in words:
...
def sort_last(lists):       # 2016.03.17_12:17:30 checked. prusanov
    # +++ ваш код +++    
    return sorted(lists,key = lastelem)
...
def remove_adjacent(nums):      # 2016.03.17_12:17:37 checked. prusanov
    # +++ ваш код +++
    if len(nums) == 0:
        return []        # не очень хорошо писать в Питоне в одну строку
    a =[]
    num1 = ''
    for st in nums:       # попробуйте обойтись без индексов - тем более, что Вы уже используете переменную для предыдущего значения
        if st != num1:
            num1 = st
            a.append(num1)
    return a
